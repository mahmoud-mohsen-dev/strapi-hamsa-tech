...
module.exports = async (policyContext, config, { strapi }) => {
  ...
  if (policyContext.request.path.startsWith('/graphql')) {
    ...
    requestedId =
      policyContext.request.body?.variables?.input?.id ||
      policyContext.request.body?.variables?.id;
  } else {
  ...
  console.log(
    !isNaN(Number(user.id)) &&
      !isNaN(Number(requestedId)) &&
      Number(user.id) === Number(requestedId)
  );
  ...
  console.log(
    'You are not authorized to access or modify this data.'
  );
  return false;
};
